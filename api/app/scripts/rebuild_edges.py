...
def run():
    ...
    with engine.begin() as conn:
        ...
        # 2. Create new edges table fresh (matching common schema; adjust columns to your real schema)
        log.info("Creating edges table (edges) from stop_times/trips/stages...")
        # Note: adapt data types to your real schema if different (e.g., id serial/bigint)
        conn.execute(text("""
            CREATE TABLE edges (
        ...
        # 3. Insert edges: ALL reachable stops from stop_times within same trip
        log.info("Populating multi-hop edges from stop_times/trips/stages (this may take a while)...")
        # This query produces edges between ALL stop pairs in the same trip where target comes after source
        conn.execute(text("""
            INSERT INTO edges (
                source, target,
                source_id, target_id,
            ...
            SELECT
                ...
                -- ✅ Cost based on hop count and distance
                -- Direct hops (consecutive stops) get lower cost
                -- Multi-hops get incrementally higher cost
                CASE
                    WHEN st2.stop_sequence = st1.stop_sequence + 1 THEN
                        -- Direct consecutive stops: distance-based cost
                        GREATEST(20,
                            LEAST(
                                150,
                                ST_DistanceSphere(
                    ...
                    ELSE
                        -- Multi-hop: base cost + hop penalty + distance factor
                        GREATEST(30,
                            LEAST(
                                300,
                ...
                -- ✅ Reverse cost same as forward for now
                CASE
                    WHEN st2.stop_sequence = st1.stop_sequence + 1 THEN
                        GREATEST(20,
                            LEAST(
            ...
            JOIN stop_times st2
                ...
                AND st2.stop_sequence > st1.stop_sequence  -- Target must come after source
                AND st2.stop_sequence <= st1.stop_sequence + 10  -- Limit max hops to prevent explosion
            JOIN stages s1 ON s1.stop_id = st1.stop_id
            JOIN stages s2 ON s2.stop_id = st2.stop_id
            JOIN trips t ON t.trip_id = st1.trip_id
            WHERE st1.stop_id != st2.stop_id  -- Avoid self-loops
            ORDER BY st1.trip_id, st1.stop_sequence, st2.stop_sequence;
        """))

        # 4. Add transfer edges between different routes at same stops
        ...
        conn.execute(text("""
            ...
            SELECT DISTINCT
                ...
                s1.stop_id AS target,  -- Same stop for transfers
                s1.node_id AS source_id,
                s1.node_id AS target_id,
                25.0 AS cost,  -- Transfer penalty (5 minutes)
                25.0 AS reverse_cost,
                NULL AS route_id,  -- No specific route for transfers
                NULL AS trip_id,
                0 AS stop_sequence,
                0 AS hop_count,
            ...
            FROM stages s1
            WHERE s1.node_id IN (
                -- Only create transfer edges at stops that serve multiple routes
                SELECT s.node_id
                FROM stages s
        ...
        # 5. Add destination node completion edges to ensure all nodes are reachable
        log.info("Adding destination node completion edges...")
        conn.execute(text("""
            INSERT INTO edges (
                source, target, source_id, target_id,
                cost, reverse_cost, route_id, trip_id,
                stop_sequence, hop_count, edge_type,
                created_at, updated_at
            )
            SELECT DISTINCT
                s.stop_id, s.stop_id,
                s.node_id, s.node_id,
                0.1, 0.1,  -- Very low cost self-loop
                'destination_completion', NULL,
                0, 0, 'destination',
                NOW(), NOW()
            FROM stages s
            WHERE s.node_id NOT IN (
                SELECT DISTINCT target_id FROM edges WHERE route_id IS NOT NULL
            );
        """))

        # 6. Deduplicate exact duplicates
        log.info("Removing exact duplicate edges if any...")
        conn.execute(text("""
            DELETE FROM edges a
            USING (
                SELECT MIN(ctid) as keep_ctid, source_id, target_id, route_id, trip_id, hop_count
                FROM edges
                GROUP BY source_id, target_id, route_id, trip_id, hop_count
                HAVING COUNT(*) > 1
            ) d
        ...
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_edges_trip_id ON edges(trip_id)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_edges_seq ON edges(stop_sequence)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_edges_hop_count ON edges(hop_count)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_edges_edge_type ON edges(edge_type)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_stages_node_id ON stages(node_id)"))
        conn.execute(text("ANALYZE edges"))

        # 8. Final sanity counts
        edges_count = conn.execute(text("SELECT COUNT(*) FROM edges")).scalar()
        direct_edges = conn.execute(text("SELECT COUNT(*) FROM edges WHERE edge_type = 'direct'")).scalar()
        multihop_edges = conn.execute(text("SELECT COUNT(*) FROM edges WHERE edge_type = 'multi_hop'")).scalar()
        transfer_edges = conn.execute(text("SELECT COUNT(*) FROM edges WHERE edge_type = 'transfer'")).scalar()
        destination_edges = conn.execute(text("SELECT COUNT(*) FROM edges WHERE edge_type = 'destination'")).scalar()
        stages_count = conn.execute(text("SELECT COUNT(*) FROM stages")).scalar()

        log.info(f"Edges rebuilt. Total: {edges_count}")
        log.info(f"  - Direct edges: {direct_edges}")
        log.info(f"  - Multi-hop edges: {multihop_edges}")
        log.info(f"  - Transfer edges: {transfer_edges}")
        log.info(f"  - Destination edges: {destination_edges}")
        log.info(f"  - Stages count: {stages_count}")

    log.info("Edges rebuild complete. OLD edges are available as edges_old if you need to rollback.")
    log.info("Next: update your routing code to use edges.source_id/target_id with pgr_dijkstra and re-run tests.")
